...
def infer_genpose2(data_path: str,
                   save_dir: str):
    ######################################## PARAMETERS ########################################
    ...
    VISUALIZE = False                                          # Show window (requires display)
    SAVE_DIR = save_dir                       # Save rendered images when VISUALIZE is False
    ######################################## PARAMETERS ########################################
    ...
    for index, color_image in enumerate(color_images):
        ...
        if VISUALIZE:
            ...
            cv2.imwrite(os.path.join(SAVE_DIR, f"infer_{index:06d}.png"), color_image_w_pose)
    if VISUALIZE:
        cv2.destroyAllWindows()
    return pose, length
